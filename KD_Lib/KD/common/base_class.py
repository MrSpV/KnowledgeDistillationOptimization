...
class BaseClass:
    ...
    def train_teacher(
        ...
        for ep in range(epochs):
            ...
            false_positive = 0
            false_negative = 0
            for (data, label, path) in self.train_loader:

                def closure():
                    loss1 = criterion(self.teacher_model(data), label)
                    loss1.mean().backward()
                    return loss1
                ...
                if pred[0].cpu().numpy() == 1 and label.cpu().numpy()[0] == 1:
                    true_positive += 1
                elif pred[0].cpu().numpy() == 0 and label.cpu().numpy()[0] == 1:
                    false_negative += 1
                elif pred[0].cpu().numpy() == 0 and label.cpu().numpy()[0] == 0:
                    false_positive += 1
                ...
                loss.mean().backward()
                self.optimizer_teacher.step(closure)
            ...
            print(
                "Epoch: {}, Loss: {}, Accuracy: {}, Precision: {}, Recall: {}".format(
                    ep + 1, epoch_loss, epoch_acc, (true_positive / (true_positive + false_positive)), true_positive / (true_positive + false_negative)
                )
            )
    ...
    def obtine_predictie(self, tsr):
        pred = tsr.argmax(dim=1, keepdim=True)
        predictie = pred.cpu().numpy()[0][0]
    ...
    def _train_student3(
        ...
        for ep in range(epochs):
            ...
            _, epoch_val_acc = self._evaluate_model2(self.student_model, verbose=True)
    ...
    def train_student(
        ...
        :param save_model_pth (str): Path where you want to save the student model
        """
        self._train_student3(epochs, plot_losses, save_model, save_model_pth)
    ...
    def _evaluate_model(self, model, verbose=True):
        ...
        with torch.no_grad():
            for data, target, path in self.val_loader:
    ...
    def _evaluate_model2(self, model, verbose=True):
        """
                Evaluate the given model's accuaracy over val set.
        For internal use only.
    ...
    def evaluate(self, teacher=False):
        ...
        _, accuracy = self._evaluate_model2(model)
