...
class I3Res50(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, use_nl=False, criterion=None):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=20, temp_conv=[0, 1, 0], temp_stride=[1, 1, 1])
    ...
    def forward(self, batch):
        pred = "ciu ciu"
        if batch.dim() == 5:
            pred = self.forward_single(batch)
            return pred

        elif batch.dim() == 7:
            pred = self.forward_multi(batch)

        return pred #, loss_dict

#-----------------------------------------------------------------------------------------------#

def i3_res50(num_classes, criterion=criterion):
    net = I3Res50(num_classes=num_classes, use_nl=False, criterion=criterion)
...
def i3_res50_nl(num_classes, criterion=criterion):
    net = I3Res50(num_classes=num_classes, use_nl=True, criterion=criterion)
    state_dict = torch.load('checkpoints/checkpoint_model6_4_99.pth', map_location='cuda:0')
